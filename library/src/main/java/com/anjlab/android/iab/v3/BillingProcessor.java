...
public class BillingProcessor extends BillingBase
{
	...
	private static final String LOG_TAG = "iabv5";
	...
	{
		handler.postDelayed(() -> initialize(), reconnectMilliseconds);
	...
	**/
							 ...
							 String purchaseType)
	{
		...
		try
		{
			...
			billingService.querySkuDetailsAsync(
					params,
					(billingResult, skuList1) -> {

						if (skuList1 != null && !skuList1.isEmpty())
						{
							startPurchaseFlow(activity, skuList1.get(0), oldProductId);
						}
						else
						{
	...
	public boolean handleActivityResult(int requestCode, int resultCode, Intent data) {
		if (requestCode != Constants.PURCHASE_FLOW_REQUEST_CODE) {
			return false;
		}
		if (data == null) {
